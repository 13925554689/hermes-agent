...
class MoAChatCompletions:
    ...
    def create(self, **api_kwargs: Any) -> Any:
        ...
        candidates: list[dict[str, str]] = [aggregator]
        try:
            from hermes_cli.config import load_config as _lc
            _fb = _lc().get("fallback_providers") or []
            _agg_prov = str(aggregator.get("provider") or "").strip().lower()
            for _slot in _fb:
                if not isinstance(_slot, dict):
                    continue
                _p = str(_slot.get("provider") or "").strip()
                _m = str(_slot.get("model") or "").strip()
                if not _p:
                    continue
                # Skip the aggregator's own provider (it already failed) and any
                # MoA preset (cannot recursively aggregate).
                if _p.strip().lower() == _agg_prov or _p.strip().lower() == "moa":
                    continue
                candidates.append({"provider": _p, "model": _m})
        except Exception as _exc:  # pragma: no cover - defensive
            logger.debug("MoA aggregator fallback chain load failed: %s", _exc)

        last_exc: Exception | None = None
        for _idx, _slot in enumerate(candidates):
            try:
                _resp = _agg_call(_slot)
                if _idx > 0:
                    logger.warning(
                ...
                last_exc = _exc
                logger.warning(
                    "MoA aggregator slot %s failed: %s", _slot_label(_slot), _exc
                )
                continue
